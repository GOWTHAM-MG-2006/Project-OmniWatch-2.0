"""
OmniWatch 2.0 — Integrations (GitLab)
Component: GitLabIntegration Tests
Layer: Enterprise
Phase: 6
Purpose: Tests for GitLab issue creation, deployment linking, and webhook handling
Inputs: Mock HTTP responses and HMAC signatures
Outputs: Test assertions for issue creation, labels, and webhook verification
"""

import hashlib
import hmac

import pytest
from unittest.mock import MagicMock, patch

from integrations.gitlab_integration import GitLabIntegration


@pytest.fixture
def integration():
    """GitLabIntegration with mocked HTTP."""
    return GitLabIntegration(
        api_base="https://gitlab.com/api/v4",
        token="glpat-test-token-123",
        webhook_secret="my_webhook_secret",
    )


class TestCreateIssue:
    """Tests for creating GitLab issues from OmniWatch incidents."""

    @patch("integrations.gitlab_integration.requests.post")
    def test_create_issue(self, mock_post):
        mock_response = MagicMock()
        mock_response.status_code = 201
        mock_response.json.return_value = {"iid": 42}
        mock_post.return_value = mock_response

        intg = GitLabIntegration(
            api_base="https://gitlab.com/api/v4",
            token="glpat_test_token",
            webhook_secret="secret",
        )

        result = intg.create_issue(
            title="[P1] Database cascade failure",
            description="PostgreSQL p99 latency exceeded 1240ms",
            labels=["incident", "P1"],
        )

        assert result["iid"] == 42
        mock_post.assert_called_once()

    @patch("integrations.gitlab_integration.requests.post")
    def test_create_issue_with_severity(self, mock_post):
        mock_response = MagicMock()
        mock_response.status_code = 201
        mock_response.json.return_value = {"iid": 43}
        mock_post.return_value = mock_response

        intg = GitLabIntegration(
            api_base="https://gitlab.com/api/v4",
            token="glpat_test_token",
            webhook_secret="secret",
        )

        result = intg.create_issue(
            title="[P2] Memory leak detected",
            description="Background worker memory growing",
            labels=[],
            severity="P2",
        )

        assert result["iid"] == 43
        call_kwargs = mock_post.call_args
        payload = call_kwargs[1]["json"]
        assert "incident" in payload["labels"]
        assert "high" in payload["labels"]


class TestWebhookVerification:
    """Tests for HMAC webhook signature verification."""

    def test_verify_webhook_valid(self):
        intg = GitLabIntegration(
            api_base="https://gitlab.com/api/v4",
            token="glpat_test_token",
            webhook_secret="test_secret",
        )

        payload = b'{"action":"opened","merge_request":{"iid":1}}'
        expected_sig = hmac.new(
            b"test_secret", payload, hashlib.sha256
        ).hexdigest()

        assert intg.verify_webhook(payload, expected_sig) is True
